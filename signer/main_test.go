...
func TestSigner(t *testing.T) {
	...
	expectedTime := 3000 * time.Millisecond //3 * time.Second
	...
	if int(OverheatLockCounter) != len(inputData) ||
		...
		t.Errorf("not enough hash-func calls %d, %d, %d", OverheatLockCounter, DataSignerMd5Counter, DataSignerCrc32Counter)
	}
